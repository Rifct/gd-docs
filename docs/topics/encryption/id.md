...
On Android, your UDID is generated by making a UUIDv4 from your SSAID ([`Settings.Secure.ANDROID_ID`](https://developer.android.com/reference/android/provider/Settings.Secure#ANDROID_ID)). However, there's a common bug with the SSAID returning the value `9774d56d682e549c` on multiple devices. If the SSAID matches that string, a random UUIDv4 is generated instead. What's interesting is that the UDID on Android is one of the few parts of the Java code, so it's easily decompilable.
